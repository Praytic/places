...
interface ManageMapsDialogProps {
  ...
  onMapsUpdated: () => void;
  onClose: () => void;
  initialEditingMapId?: string | null;
}

const ManageMapsDialog: React.FC<ManageMapsDialogProps> = ({
  userEmail,
  userMaps,
  visibleMapIds,
  ...
  const [editingMapId, setEditingMapId] = useState<string | null>(initialEditingMapId);
  ...
  const handleStartEditing = (map: PlaceMapWithRole) => {
  ...
  const handleToggleVisibility = (mapId: string) => {
